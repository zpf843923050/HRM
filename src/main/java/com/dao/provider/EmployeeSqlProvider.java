        ...
        return new SQL() {
            {
                ...
                FROM(EMPLOYEETABLE);
                if (params.get("employee") != null) {
                    Employee dept = (Employee) params.get("employee");
                    if (dept.getDept() != null && dept.getDept().getId() != null
                            && dept.getDept().getId() != 0) {
                        WHERE("DEPT_ID=#{employee.dept.id}");
                    }
                    if (dept.getJob() != null && dept.getJob().getId() != null
                            && dept.getJob().getId() != 0) {
                        WHERE("JOB_ID=#{employee.job.id}");
                    }
                    ...
                    if (dept.getCardId() != null && !dept.getCardId().equals("")) {
                        WHERE("CARD_ID LIKE CONCAT('%,#{employee.cardId},'%')");
                    }
                    if (dept.getSex() != null && dept.getSex() != 0) {
                        WHERE("sex=#{employee.sex}");
                    }
                }
            }
